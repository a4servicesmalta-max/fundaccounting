...
- **Final working product:** 220/220 tests green, tsc clean; full 2022 reconciliation validated (TB ties €83,077,192.72, share capital exact, **P&L clean**); accounting test-and-fix loop verdict **shippable** (approval gate held on every ingest, 0 P1). Latest commit `314ec74` on `main`.
- **NH-0 fixed (2026-06-21):** investment cash-leg double-count closed. A share/loan purchase posts Dr 030/032 / Cr 1010; when the same payment also lands on an imported bank statement, the statement line is now deterministically matched (same currency/magnitude/direction/near-date) via `matchedInvestmentDraftId` and **excluded from the GL** so the cash + asset are each counted once. New `src/bank/investment-settle.ts`, wired into `approveDraft`/`approveAll` and bank ingest; 4 regression tests incl. the double-count precondition.
- **Remaining NEEDS-HUMAN (policy/judgement, deliberately NOT auto-encoded):** NH-1 AR/AP VAT model, NH-2 dividend WHT, NH-3 FVOCI recycling, NH-4 loan-interest account, NH-5 credit-note reversal. See `qa/JE_LOOP_RESULTS.md` / `test-runs/je-genloop-2026-06-21.md`. These are design calls awaiting the user.
